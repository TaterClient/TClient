		...
		for(int i = 0; i < Num; i++)
		{
			...
			if(Item.m_Type == NETOBJTYPE_CLIENTINFO)
			{
				...
				if(ClientId < MAX_CLIENTS)
				{
					...
					m_TClient.HandleClientCountry(pClient->m_Country, ClientId); //TClient
...
void CGameClient::SendStartInfo7(bool Dummy)
{
	...
	Msg.m_Country = m_TClient.ReplaceCountryFlagWithCustomClientId(Dummy ? Config()->m_ClDummyCountry : Config()->m_PlayerCountry);
...
void CGameClient::SendInfo(bool Start)
{
	...
	if(Start)
	{
		...
		Msg.m_Country = m_TClient.ReplaceCountryFlagWithCustomClientId(g_Config.m_PlayerCountry);
...
void CGameClient::SendDummyInfo(bool Start)
{
	...
	if(Start)
	{
		...
		Msg.m_Country = m_TClient.ReplaceCountryFlagWithCustomClientId(g_Config.m_ClDummyCountry);
